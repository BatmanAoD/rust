        ...
        if self.optimizations.and_stars.remove(&location) {
            ...
            let new_place = match *rvalue {
                Rvalue::Ref(_, _, Place {
                    ...
                    let mut projection1 = projection.to_vec();
                    let projection2 = projection1.split_off(projection1.len() - 1);

                    let place = Place {
                        // Replace with dummy
                        base: mem::replace(base, PlaceBase::Local(Local::new(0))),
                        projection: projection1.into_boxed_slice(),
                    };
                    *projection = projection2.into_boxed_slice();
